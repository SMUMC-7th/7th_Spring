...
import com.example.umc7th.kakao.dto.OAuth2DTO;
...
public class OAuth2ServiceImpl implements OAuth2Service {
    ...
    public MemberResponseDTO.MemberTokenDTO login(String code) {
        ...
        OAuth2DTO.KakaoProfile profile = webClient.get()
                .uri(userInfoURI)
                .header("Authorization", "Bearer " + oAuth2TokenDTO.getAccess_token())
                .retrieve()
...
//        String email = profile.getId().toString();
        ...
        if (email == null) {
            throw new MemberException(MemberErrorCode.OAUTH_USER_EMAIL_FAIL); //이메일 정보가 없음
        }

        //이메일 존재시 member에 넣고 아니면 이메일을 가져와서 새로 저장하기
